  ...
  return (
    <section>
      <div className="mb-4 md:mb-6">
        <h1 className="text-lg md:text-xl font-semibold">Knowledge</h1>
      ...
      <div className="grid gap-4 md:gap-6 grid-cols-1 lg:grid-cols-2">
        ...
        >
          <h2 className="text-xs md:text-sm font-semibold">Ingest document</h2>
          <input
            value={title}
        ...
        <form
          ...
          className="rounded-lg border border-slate-200 bg-white p-3 md:p-4"
        >
          <h2 className="text-xs md:text-sm font-semibold">Ask the knowledge base</h2>
          <textarea
            value={question}
          ...
          {queryError && (
            <div className="mt-3 rounded-md bg-red-50 p-2 md:p-3 text-xs md:text-sm text-red-700">
              {queryError}
            </div>
          )}

          {answer && (
            <div className="mt-3 rounded-md bg-slate-50 p-2 md:p-3 text-xs md:text-sm">
              <p
                className={
      ...
      {/* Documents table */}
      <h2 className="mb-2 mt-6 md:mt-8 text-xs md:text-sm font-semibold">Documents</h2>
      {loading ? (
        <div className="text-xs md:text-sm text-slate-500">Loading documents...</div>
      ) : docs.length === 0 ? (
        <div className="rounded-lg border border-dashed border-slate-300 bg-slate-50 p-6 md:p-10 text-center text-xs md:text-sm text-slate-500">
        ...
        <div className="overflow-x-auto rounded-lg border border-slate-200 bg-white">
          <table className="min-w-full divide-y divide-slate-200 text-left text-xs md:text-sm">
            <thead className="bg-slate-50 text-slate-600">
              <tr>
                <th className="px-3 md:px-4 py-2 md:py-3 font-medium">Title</th>
                <th className="px-3 md:px-4 py-2 md:py-3 font-medium">Status</th>
                <th className="px-3 md:px-4 py-2 md:py-3 font-medium">Chunks</th>
                <th className="px-3 md:px-4 py-2 md:py-3 font-medium">Created</th>
            ...
            <tbody className="divide-y divide-slate-200">
              {docs.map((d) => (
                <tr key={d.id} className="hover:bg-slate-50">
                  <td className="px-3 md:px-4 py-2 md:py-3">{d.title}</td>
