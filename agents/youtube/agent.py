...
class YoutubeAgent:
    ...
    async def _transcript(self, request: TaskRequest) -> AgentResponse:
        url = str(
            ...
            or request.payload.get("video_id")
            or request.payload.get("video_url")
            or ""
        ).strip()
        if not url:
            return AgentResponse(
                ...
                status=AgentResponseStatus.REJECTED,
                error=ErrorDetail(
                    code="VALIDATION_ERROR", message="payload.url or payload.video_id required"
                ),
            )
        ...
        try:
            ...
            if content.strip() and "mock content" not in content:
                return AgentResponse(
                    ...
                    agent=self.descriptor.qualified_name,
                    status=AgentResponseStatus.SUCCESS,
                    result={"video_id": vid, "transcript": content[:8000]},
                )
        except Exception as e:
            return AgentResponse(
                task_id=request.task_id,
        ...
        return AgentResponse(
            ...
            agent=self.descriptor.qualified_name,
            status=AgentResponseStatus.SUCCESS,
            result={"video_id": vid, "transcript": f"[mock transcript for {vid}]", "mock": True},
        )
    ...
    async def _summarize(self, request: TaskRequest) -> AgentResponse:
        ...
        if not text and not url:
            return AgentResponse(
                ...
                status=AgentResponseStatus.REJECTED,
                error=ErrorDetail(
                    code="VALIDATION_ERROR", message="payload.text or payload.url required"
                ),
            )
        if not text and url:
            # fetch transcript first
            tmp = await self._transcript(
                TaskRequest(
                    task_id=request.task_id,
                    action="transcript",
                    payload={"url": url},
        ...
        try:
            summary = await self._llm.generate(
                prompt=f"Summarize this youtube transcript concisely:\n{text[:7000]}",
                system="You are a youtube summarizer.",
            )
