...
class CalendarAgent:
    def __init__(self, descriptor: AgentDescriptor | None = None, llm: LLMProvider | None = None) -> None:
        self.descriptor = descriptor or AgentDescriptor(
            ...
            capabilities=frozenset({"calendar.list_events", "calendar.create_event", "calendar.delete_event"}),
        )
    ...
    async def handle(self, request: TaskRequest) -> AgentResponse:
        ...
        if not _HAS_GOOGLE:
            return AgentResponse(task_id=request.task_id, agent=self.descriptor.qualified_name, status=AgentResponseStatus.REJECTED, error=ErrorDetail(code="CONFIG_ERROR", message="Google Calendar not configured (missing google libraries)"))
    ...
    async def _list_events(self, request: TaskRequest, creds: Any) -> AgentResponse:
        ...
        def _call() -> list[dict[str, Any]]:
            ...
            resp = svc.events().list(calendarId=calendar_id, maxResults=max_results, singleEvents=True, orderBy="startTime").execute()
            return resp.get("items", [])

        try:
            items = await asyncio.to_thread(_call)
            return AgentResponse(task_id=request.task_id, agent=self.descriptor.qualified_name, status=AgentResponseStatus.SUCCESS, result={"events": items, "count": len(items)})
        except Exception as e:
            return AgentResponse(task_id=request.task_id, agent=self.descriptor.qualified_name, status=AgentResponseStatus.FAILED, error=ErrorDetail(code="CALENDAR_ERROR", message=str(e)))
    ...
    async def _create_event(self, request: TaskRequest, creds: Any) -> AgentResponse:
        ...
        if not summary:
            return AgentResponse(task_id=request.task_id, agent=self.descriptor.qualified_name, status=AgentResponseStatus.REJECTED, error=ErrorDetail(code="VALIDATION_ERROR", message="payload.summary required"))
        ...
        try:
            ...
            return AgentResponse(task_id=request.task_id, agent=self.descriptor.qualified_name, status=AgentResponseStatus.SUCCESS, result={"event": ev})
        except Exception as e:
            return AgentResponse(task_id=request.task_id, agent=self.descriptor.qualified_name, status=AgentResponseStatus.FAILED, error=ErrorDetail(code="CALENDAR_ERROR", message=str(e)))
    ...
    async def _delete_event(self, request: TaskRequest, creds: Any) -> AgentResponse:
        ...
        if not event_id:
            return AgentResponse(task_id=request.task_id, agent=self.descriptor.qualified_name, status=AgentResponseStatus.REJECTED, error=ErrorDetail(code="VALIDATION_ERROR", message="payload.event_id required"))
        calendar_id = str(request.payload.get("calendar_id", "primary"))
        ...
        try:
            ...
            return AgentResponse(task_id=request.task_id, agent=self.descriptor.qualified_name, status=AgentResponseStatus.SUCCESS, result={"deleted": True, "event_id": str(event_id)})
        except Exception as e:
            return AgentResponse(task_id=request.task_id, agent=self.descriptor.qualified_name, status=AgentResponseStatus.FAILED, error=ErrorDetail(code="CALENDAR_ERROR", message=str(e)))
