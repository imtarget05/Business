...
def _food_summary_is_safe(summary: str, results: list[dict]) -> bool:
    ...
    if not summary:
        return True
    _blob = " ".join(f"{r.get('title', '')} {r.get('snippet', '')}" for r in results).lower()
    ...
    try:
        ...
        if not _food_summary_is_safe(_ans, results):
            try:
                ...
                _model = getattr(llm, "name", "unknown")
                log_llm_usage(
                    _model, _prompt, _ans, _dt, cache_hit=False, tag="food_summary_rejected"
                )
            except Exception:
                pass
            return ""
...
class MonitoringBot:
    ...
    def __init__(self, config: TelegramConfig) -> None:
        ...
        if TELEGRAM_AVAILABLE:
            ...
            self.bot = _SanitizingBot(
                _StubBot(token=config.bot_token)
            )  # Use stub for testing/offline
        ...
        self._last_jobsearch: dict[
            int, list[dict]
        ] = {}  # chat_id -> final_list from last run (for send-unconfirmed)
    ...
    async def initialize(self) -> None:
        ...
        self.app.add_handler(CallbackQueryHandler(self._pagination_callback, pattern=r"^pg:"))
    ...
    async def send_message(
        self, text: str, chat_id: int | None = None, parse_mode: str = ParseMode.MARKDOWN
    ) -> None:
    ...
    async def send_health_alert(self, health_dict: dict[str, Any]) -> None:
        ...
        alert_text = "🚨 *System Health Alert*\n\n"
        ...
        for check in health_dict.get("checks", []):
            ...
            icon = {"ok": "✅", "warning": "⚠️", "error": "🚨", "unavailable": "❓"}.get(
                status, "❓"
            )
    ...
    def _callback_user_id(self, query: Any) -> int:
        ...
        if uid is None:
            ...
            uid = getattr(getattr(msg, "chat", None), "id", None) or getattr(msg, "chat_id", None)
        try:
            return int(uid)
        except (TypeError, ValueError):
            return 0
    ...
    async def _handle_quick_reply(
        ...
        try:
            self.sessions.update(uid, last_capability=AWAITING_FOOD_QUERY, page=0, results=[])
        except Exception:
            pass
        await self._reply(
            update,
    ...
    async def _edit_callback_text(self, query: Any, text: str, reply_markup: Any = None) -> Any:
    ...
    async def _pagination_callback(
        ...
        if not items:
            await self._edit_callback_text(
                query_obj,
                "⌛ Danh sách này đã hết hạn (30 phút). Bạn gửi lại câu hỏi để mình tra lại nhé.",
            )
            return
        ...
        text = self._format_page(items, page, header=PAGE_HEADER.format(query=last_query))
        markup = self._pagination_keyboard(page, total_pages, token or self._page_token(last_query))
    ...
    async def _ops_command(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        ...
        try:
            ...
            if len(text) > 4000:
                text = text[:3900] + "\n*... (đã rút gọn) ...*"
            await update.message.reply_text(
                _md_to_telegram_html(_sanitize_text(text)), parse_mode=ParseMode.HTML
            )
        except Exception as e:
    ...
    async def _advisory_command(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        ...
        source = (
            "chỉ định"
            if explicit_persona
            else ("tự động nhận diện" if select_persona(question) else "mặc định (hormozi)")
        )
        ...
        try:
            ...
            if re.match(r"^[a-zA-Z0-9_-]{6,}$", raw.strip()) and "@" not in raw:
                try:
                    ...
                    if (
                        gresp.status.value == "success"
                        and gresp.result
                        and gresp.result.get("messages")
                    ):
    ...
    async def _help_command(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        ...
        if hasattr(update, "callback_query") and update.callback_query:
            await update.callback_query.edit_message_text(
                help_text, parse_mode=ParseMode.MARKDOWN, reply_markup=self._main_menu_keyboard()
            )
        else:
            await self._reply(
                update,
    ...
    def _main_menu_keyboard(self):
        ...
        return InlineKeyboardMarkup(
            [
                [
                    InlineKeyboardButton("📚 Knowledge", callback_data="kb"),
                    InlineKeyboardButton("📥 Ops Hub", callback_data="ops"),
                ],
                [
                    InlineKeyboardButton("💡 Advisory", callback_data="advisory"),
                    InlineKeyboardButton("📄 Proposal", callback_data="sales"),
                ],
                [
                    InlineKeyboardButton("📊 Competitor", callback_data="compete"),
                ...
                [
                    ...
                    InlineKeyboardButton("🛠 Công cụ", callback_data="tools"),
                ],
        ...
        try:
            if d == "health":
                ...
                await q.edit_message_text(
                    txt2, parse_mode=ParseMode.MARKDOWN, reply_markup=self._main_menu_keyboard()
                )
            elif d == "report":
                ...
                if len(md) > 3500:
                    md = md[:3500] + "\n..."
                await q.edit_message_text(
                    md, parse_mode=ParseMode.MARKDOWN, reply_markup=self._main_menu_keyboard()
                )
            elif d == "research":
                await q.edit_message_text(
                    "🔍 *Nghiên cứu*\nNhập: /research <câu hỏi>\nVD: `/research LangGraph là gì?`",
                    parse_mode=ParseMode.MARKDOWN,
                    reply_markup=self._main_menu_keyboard(),
                )
            elif d == "gmail":
                await q.edit_message_text(
                    "📧 *Gmail*\n• /gmail hoặc gõ 'check mail'\n• Gửi: chỉ tới tanmainguyenbinh@gmail.com\n• Dùng Gmail agent (list/search/send)",
                    parse_mode=ParseMode.MARKDOWN,
                    reply_markup=self._main_menu_keyboard(),
                )
            elif d == "calendar":
                try:
                    ...
                    await q.edit_message_text(
                        f"📅 *Calendar (5 sự kiện gần nhất)*\n{out}",
                        parse_mode=ParseMode.MARKDOWN,
                        reply_markup=self._main_menu_keyboard(),
                    )
                except Exception as e2:
                    await q.edit_message_text(
                        f"📅 Calendar: {e2}\nDùng: /calendar hoặc hỏi 'lịch hôm nay?'",
                        reply_markup=self._main_menu_keyboard(),
                    )
            elif d == "youtube":
                await q.edit_message_text(
                    "🎥 *YouTube*\nNhập: /research youtube <từ khóa> hoặc hỏi 'tìm video về ...'",
                    parse_mode=ParseMode.MARKDOWN,
                    reply_markup=self._main_menu_keyboard(),
                )
            elif d == "context":
                await q.edit_message_text(
                    "🧠 *Context*\nBộ nhớ hội thoại per-org. Hỏi 'tóm tắt context' hoặc 'xóa context'",
                    parse_mode=ParseMode.MARKDOWN,
                    reply_markup=self._main_menu_keyboard(),
                )
            elif d == "supply":
                await q.edit_message_text(
                    "📦 *Supply Chain*\nPO → approval → inventory → reporting → n8n\nHỏi: 'check inventory' hoặc 'báo cáo supply chain'",
                    parse_mode=ParseMode.MARKDOWN,
                    reply_markup=self._main_menu_keyboard(),
                )
            elif d == "setup_mail":
                ...
                kb = _M(
                    [
                        [
                            _B("➕ Thêm mail", callback_data="add_mail"),
                            _B("➖ Xóa mail", callback_data="del_mail"),
                        ],
                        [
                            _B("📋 Xem danh sách", callback_data="list_mails"),
                            _B("⬅️ Menu chính", callback_data="back_menu"),
                        ],
                    ]
                )
                await q.edit_message_text(
                ...
                self._awaiting_add_mail.add(
                    q.message.chat.id if q.message and q.message.chat else 0
                )
                self._awaiting_del_mail.discard(
                    q.message.chat.id if q.message and q.message.chat else 0
                )
                await q.edit_message_text(
                    "➕ *Thêm mail*\nGửi email muốn THÊM (VD: new@gmail.com):",
                    parse_mode=ParseMode.MARKDOWN,
                )
            elif d == "del_mail":
                self._awaiting_del_mail.add(
                    q.message.chat.id if q.message and q.message.chat else 0
                )
                self._awaiting_add_mail.discard(
                    q.message.chat.id if q.message and q.message.chat else 0
                )
                ...
                from telegram import InlineKeyboardButton as _B2
                from telegram import InlineKeyboardMarkup as _M2

                kb2 = _M2(
                    [
                        [
                            _B2("➕ Thêm", callback_data="add_mail"),
                            _B2("➖ Xóa", callback_data="del_mail"),
                        ],
                        [_B2("⬅️ Quay lại", callback_data="setup_mail")],
                    ]
                )
                await q.edit_message_text(
                    f"📋 *Allowlist hiện tại*\n{lst2}",
                    parse_mode=ParseMode.MARKDOWN,
                    reply_markup=kb2,
                )
            elif d == "export":
                try:
                    ...
                    await q.message.reply_document(
                        document=open(tf, "rb"), filename="bao_cao.md", caption="📤 Xuất báo cáo"
                    )
                    await q.edit_message_text(
                        "✅ Đã xuất báo cáo (file đính kèm)",
                        reply_markup=self._main_menu_keyboard(),
                    )
                except Exception as e:
                    await q.edit_message_text(
                        f"❌ Xuất thất bại: {e}", reply_markup=self._main_menu_keyboard()
                    )
            elif d == "new_session":
                try:
                    self._seen_chats.discard(
                        q.message.chat.id if q.message and q.message.chat else 0
                    )
                    self._awaiting_add_mail.discard(
                        q.message.chat.id if q.message and q.message.chat else 0
                    ...
                    self._research_awaiting.pop(
                        q.message.chat.id if q.message and q.message.chat else 0, None
                    )
                    await q.edit_message_text(
                        "🔄 *Session mới*\\nĐã xóa context, bắt đầu lại. Gõ /start để chào lại.",
                        parse_mode=ParseMode.MARKDOWN,
                        reply_markup=self._main_menu_keyboard(),
                    )
                except Exception as e:
                    await q.edit_message_text(
                        f"❌ Lỗi: {e}", reply_markup=self._main_menu_keyboard()
                    )
            elif d == "back_menu":
                await q.edit_message_text(
                    "Chọn chức năng:", reply_markup=self._main_menu_keyboard()
                )
            elif d == "open_menu":
                await q.edit_message_text(
                ...
                kb = _M(
                    [
                        [
                            _B("⚙️ Setup Mail", callback_data="setup_mail"),
                            _B("⬅️ Quay lại", callback_data="back_menu"),
                        ],
                    ]
                )
                await q.edit_message_text(
                    "🛠 *Công cụ* — chọn thao tác:", parse_mode=ParseMode.MARKDOWN, reply_markup=kb
                )
            elif d == "help":
                ...
                await q.edit_message_text(
                    "❌ Đã hủy JobSearch. Gửi lại brief khi bạn sẵn sàng.",
                    reply_markup=self._main_menu_keyboard(),
                )
                ...
                await q.edit_message_text(
                    "❌ Đã hủy YouTube Trending. Gửi lại brief khi bạn sẵn sàng.",
                    reply_markup=self._main_menu_keyboard(),
                )
                ...
                if not pending:
                    await q.edit_message_text(
                        "⚠️ Không tìm thấy brief YouTube. Gửi lại.",
                        reply_markup=self._main_menu_keyboard(),
                    )
                    return
                target_mail = pending.get("target_mail", "tanmainguyenbinh@gmail.com")
                await q.edit_message_text(
                    f"🎥 Đang lấy YouTube Trending Việt Nam cho *{target_mail}* (1-2 phút)...",
                    parse_mode=ParseMode.MARKDOWN,
                )
                import asyncio as _aioY
                import datetime as _dtY
                import re as _reY
                import uuid as _uuidY

                import httpx as _httpxY
                ...
                async def _do_youtube():
                    try:
                        ...
                        queries = [
                            "trending Vietnam",
                            "nhạc trending Vietnam",
                            "gaming trending Vietnam",
                        ]
                        all_vids: list[dict] = []
                        for _q in queries:
                            try:
                                _req = TaskRequest(
                                    task_id=_uuidY.uuid4(),
                                    domain=Domain.YOUTUBE,
                                    action="search",
                                    payload={"query": _q, "limit": 5},
                                    context=TaskContext(
                                        organization_id=_uuidY.UUID(
                        ...
                        if not all_vids:
                            try:
                                _req2 = TaskRequest(
                                    ...
                                    action="web_search",
                                    payload={
                                        "query": "site:youtube.com trending Vietnam",
                                        "limit": 10,
                                    },
                                    context=TaskContext(
                                        organization_id=_uuidY.UUID(
                                ...
                                _desc2, _handler2 = ctn.registry.get_by_capability(
                                    "research.web_search"
                                )
                        ...
                        try:
                            await context.bot.send_message(
                                chat_id=chat_id2,
                                text=f"🔎 Đã search YouTube {len(all_vids)} nguồn, đang verify link còn xem được...",
                            )
                        except Exception:
                            pass
                        seen: set[str] = set()
                        ...
                        for it in all_vids:
                            ...
                            if u and u not in seen and "youtube.com/watch" in u:
                                ...
                                uniq.append(it)
                        verified: list[dict] = []
                        now = _dtY.datetime.now(_dtY.UTC).isoformat()
                        for it in uniq[:15]:
                            url = it.get("url", "")
                            ...
                            try:
                                async with _httpxY.AsyncClient(
                                    timeout=10,
                                    ...
                                    if (
                                        _r.status_code == 200
                                        and "Video unavailable" not in _r.text
                                        and "Private video" not in _r.text
                                    ):
                                        m_t = _reY.search(
                                            r"<title[^>]*>(.*?)</title>",
                                            _r.text,
                                            flags=_reY.IGNORECASE | _reY.DOTALL,
                                        )
                                        if m_t:
                                            t = (
                                                _reY.sub(r"<[^>]+>", "", m_t.group(1))
                                                .replace(" - YouTube", "")
                                                .strip()
                                            )
                        ...
                        if not verified:
                            try:
                                await context.bot.send_message(
                                    chat_id=chat_id2,
                                    text="⚠️ Không verify được video YouTube nào còn xem được. Mình không bịa link abc123.",
                                )
                            except Exception:
                                pass
                            return
                        ...
                        try:
                            await context.bot.send_message(
                                chat_id=chat_id2, text=summary[:4000], parse_mode=ParseMode.MARKDOWN
                            )
                        except Exception:
                            pass
                        try:
                            ...
                            if target_mail.lower() not in [a.lower() for a in allowed]:
                                await context.bot.send_message(
                                    chat_id=chat_id2,
                                    text=f"⚠️ {target_mail} chưa trong allowlist. Dùng /menu → ⚙️ Setup Mail để thêm.",
                                )
                            else:
                                body = (
                                    summary
                                    + "\n\n"
                                    + "\n".join(
                                        f"{v['title']} | {v['channel']} | {v['url']}"
                                        for v in verified
                                    )
                                )
                                _res = gmail_send(
                                    to=target_mail,
                                    subject=f"[Business Ops] TOP {len(verified)} YouTube Trending VN — {now[:10]}",
                                    body=body,
                                )
                                if _res.get("mode") == "DRY_RUN":
                                    await context.bot.send_message(
                                        chat_id=chat_id2,
                                        text=f"⚠️ Gmail DRY_RUN chưa gửi thật tới {target_mail}",
                                    )
                                else:
                                    await context.bot.send_message(
                                        chat_id=chat_id2,
                                        text=f"✅ Đã gửi báo cáo YouTube Trending về {target_mail} (id {_res.get('id')})",
                                    )
                        except Exception as _e:
                            try:
                                await context.bot.send_message(
                                    chat_id=chat_id2, text=f"⚠️ Gửi mail lỗi: {_e}"
                                )
                            except Exception:
                                pass
                    except Exception as e2:
                        try:
                            await context.bot.send_message(
                                chat_id=chat_id2, text=f"❌ YouTube lỗi: {e2}"
                            )
                        except Exception:
                            pass
                ...
                if not _final:
                    await q.edit_message_text(
                        "⚠️ Không có kết quả để gửi. Hãy chạy tìm kiếm lại.",
                        reply_markup=self._main_menu_keyboard(),
                    )
                    return
                target_mail = (self._pending_jobsearch.get(chat_id2, {}) or {}).get(
                    "target_mail"
                ) or "tanmainguyenbinh@gmail.com"
                try:
                    from integrations.google_client import gmail_send
                    from packages.config.settings import get_settings
                    ...
                    allowed = get_settings().gmail_allowed_recipients or []
                    _summary_lines = "\n".join(
                        f"{i + 1}. {j.get('job_title', '?')} — {j.get('company', '?')} | {j.get('link') or j.get('url')}"
                        for i, j in enumerate(_final)
                    )
                    _body = f"**TUYỂN DỤNG — CHƯA XÁC NHẬN (gửi theo yêu cầu user)**\n\n{_summary_lines}"
                    if target_mail.lower() not in [a.lower() for a in allowed]:
                        await context.bot.send_message(
                            chat_id=chat_id2,
                            text=f"⚠️ {target_mail} chưa trong allowlist. Dùng /menu → ⚙️ Setup Mail → ➕ Thêm mail trước.",
                        )
                    else:
                        _res = gmail_send(
                            to=target_mail,
                        ...
                        if _res.get("mode") == "DRY_RUN":
                            ...
                            await context.bot.send_message(
                                chat_id=chat_id2,
                                text=f"📨 Đã gửi (chưa verify) TOP {len(_final)} về {target_mail} (id {_res.get('id')})",
                            )
                except Exception as _e:
                    try:
                        await context.bot.send_message(
                ...
                await q.edit_message_text(
                    "🔍 Mở rộng nguồn: thử lại với thêm từ khóa ngành + nhiều trang (TopCV, ITviec, VietnamWorks, LinkedIn, CareerBuilder)... Vui lòng gõ lại brief nếu muốn đổi tiêu chí.",
                    reply_markup=self._main_menu_keyboard(),
                )
                return
                ...
                if not pending:
                    await q.edit_message_text(
                        "⚠️ Không tìm thấy brief. Gửi lại brief AI Intern.",
                        reply_markup=self._main_menu_keyboard(),
                    )
                    return
                ...
                await q.edit_message_text(
                    _search_lbl(_n_job) + f" cho *{target_mail}*", parse_mode=ParseMode.MARKDOWN
                )
                ...
                async def _do_jobsearch_confirm():
                    try:
                        ...
                        try:
                            ...
                            _ctx_res = await _ctx_handler.handle(
                                TaskRequest(
                                    task_id=_uuid2.uuid4(),
                                    domain=Domain.RESEARCH,
                                    action="get",
                                    payload={},
                                    context=TaskContext(
                                        organization_id=_org_id, channel="telegram"
                                    ),
                                )
                            )
                            ...
                            if _ctx_items:
                                from agents.monitoring.jobsearch_filters import (
                                    context_job_keywords as _ctx_jw,
                                )
                        ...
                        for _q in queries:
                            try:
                                _req = TaskRequest(
                                    ...
                                    payload={"query": _q, "limit": 5},
                                    context=TaskContext(
                                        organization_id=_uuid2.UUID(
                                            "00000000-0000-0000-0000-000000000001"
                                        ),
                                        channel="telegram",
                                    ),
                                )
                                _desc, _handler = ctn.registry.get_by_capability(
                                    "research.web_search"
                                )
                                _resp = await _handler.handle(_req)
                                if _resp.result and _resp.result.get("results"):
                        ...
                        try:
                            await context.bot.send_message(
                                chat_id=chat_id2,
                                text=f"🔎 Đã thu thập {len(all_items)} kết quả từ các nền tảng, đang xác thực liên kết ứng tuyển và chấm điểm...",
                            )
                        except Exception:
                            pass
                        ...
                        audit: list[dict] = []
                        now = _dt2.datetime.now(_dt2.UTC).isoformat()
                        bg_keywords = [
                            "python",
                            "docker",
                            "kubernetes",
                        ...
                        for it in uniq[:25]:
                            url = it.get("url", "")
                            orig_title = (
                                it.get("title", "") or it.get("snippet", "") or url.split("/")[2]
                            )
                            title = orig_title
                            status = "UNCERTAIN"
                            ...
                            try:
                                ...
                                if _html:
                                    _vr = verify_job_listing(
                                        url, _html, now, fallback_title=orig_title
                                    )
                                    title = _vr["title"]
                                    status = _vr["status"]
                            ...
                            skill_match = (
                                sum(1 for k in bg_keywords if k in low_title)
                                / max(1, len(bg_keywords))
                                * 40
                                + 50
                            )
                            if "intern" in low_title:
                                skill_match += 10
                            if "ai" in low_title:
                                skill_match += 10
                            ...
                            if _loc_label:
                                ...
                                loc = (
                                    "Hồ Chí Minh"
                                    if "hcm" in low_title or "ho chi minh" in low_title
                                    else (
                                        "Hà Nội"
                                        if "hanoi" in low_title or "ha noi" in low_title
                                        else "Vietnam"
                                    )
                                )
                            ...
                            if " - " in title:
                                company = (
                                    title.split(" - ")[1].split("|")[0].split("-")[0].strip()[:40]
                                )
                            elif " tại " in title.lower():
                                company = (
                                    title.lower()
                                    .split(" tại ")[1]
                                    .split("|")[0]
                                ...
                                company = (
                                    orig_title.split("—")[0].strip()[:40]
                                    if "—" in orig_title
                                    else title.split("|")[0].strip()[:40]
                                )
                            ...
                            job = {
                                ...
                                "required_skills": ", ".join(
                                    [k for k in bg_keywords if k in low_title][:5]
                                ),
                                "experience": "Intern",
                                "link": url,
                                "checked_at": now,
                        ...
                        _verified_count = sum(
                            1 for j in final_list if j.get("status") == "VERIFIED"
                        )
                        try:
                            _drop_log = (
                        ...
                        try:
                            _base = (
                                _pl2.Path("D:/Business Ops Agent Swarm")
                                if _pl2.Path(
                                    "D:/Business Ops Agent Swarm/job_search_results.json"
                                ).parent.exists()
                                else _pl2.Path(".")
                            )
                            (_base / "job_search_results.json").write_text(
                                _json2.dumps(uniq, ensure_ascii=False, indent=2), encoding="utf-8"
                            )
                            (_base / "verified_jobs.json").write_text(
                                _json2.dumps(final_list, ensure_ascii=False, indent=2),
                                encoding="utf-8",
                            )
                            (_base / "job_audit_log.json").write_text(
                                _json2.dumps(audit, ensure_ascii=False, indent=2), encoding="utf-8"
                            )
                        except Exception:
                            pass
                        if not final_list:
                        ...
                        for idx, j in enumerate(final_list, 1):
                            why = (
                                ...
                                or "Đúng ngành AI Intern, khớp nền tảng ML/Cloud"
                            )
                        ...
                        if _gate_decision["action"] == "ASK_USER":
                            try:
                                ...
                                from telegram import InlineKeyboardButton as _Bg
                                from telegram import InlineKeyboardMarkup as _Mg

                                _kb = _Mg(
                                    [
                                        [
                                            _Bg(
                                                "📨 Gửi luôn (chưa verify)",
                                                callback_data="jobsearch_send_unconfirmed",
                                            ),
                                            _Bg(
                                                "🔍 Mở rộng nguồn", callback_data="jobsearch_expand"
                                            ),
                        ...
                        try:
                            ...
                            email_body = (
                                ...
                                + "\n\n"
                                + "\n".join(
                                    f"{j['company']} | {j['job_title']} | {j['link']} | Match {j['match']} | {j['evidence']}"
                                    for j in final_list[:5]
                                )
                            )
                            if target_mail.lower() not in [a.lower() for a in allowed]:
                                await context.bot.send_message(
                                    chat_id=chat_id2,
                                    text=f"⚠️ {target_mail} chưa trong allowlist ({', '.join(allowed)}). Dùng /menu → ⚙️ Setup Mail → ➕ Thêm mail trước.",
                                )
                            else:
                                _tag = "VERIFIED" if _verified_count else "CHUA XAC NHAN"
                                _res = gmail_send(
                                    to=target_mail,
                                    subject=f"[Business Ops] TOP {len(final_list[:5])} JobSearch {_tag} — {now[:10]}",
                                    body=email_body,
                                )
                                if _res.get("mode") == "DRY_RUN":
                                    await context.bot.send_message(
                                    ...
                                    await context.bot.send_message(
                                        chat_id=chat_id2,
                                        text=f"✅ Đã gửi báo cáo TOP {len(final_list[:5])} {_tag} về {target_mail} (id {_res.get('id')})",
                                    )
                        except Exception as _e:
                            try:
                                await context.bot.send_message(
                        ...
                        try:
                            await context.bot.send_message(
                                chat_id=chat_id2, text=f"❌ JobSearch lỗi: {e2}"
                            )
                        except Exception:
                            pass
    ...
    async def _message_handler(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        ...
        if chat_id in self._awaiting_add_mail:
            ...
            try:
                ...
                if new_mail in allowed:
                    await update.message.reply_text(
                        f"⚠️ {new_mail} đã có trong allowlist: {', '.join(allowed)}",
                        reply_markup=self._main_menu_keyboard(),
                    )
                else:
        ...
        if chat_id in self._awaiting_del_mail:
            ...
            try:
                ...
                if del_mail not in allowed3:
                    await update.message.reply_text(
                        f"⚠️ {del_mail} không có trong allowlist: {', '.join(allowed3) if allowed3 else '(trống)'}",
                        reply_markup=self._main_menu_keyboard(),
                    )
                else:
                    ...
                    _gs3.cache_clear()
                    await update.message.reply_text(
                        f"🗑️ Đã XÓA {del_mail}\nCòn lại: {', '.join(allowed3) if allowed3 else '(trống)'}",
                        reply_markup=self._main_menu_keyboard(),
                    )
            except Exception as e:
        ...
        if chat_id in self._pending_jobsearch and not text.startswith("/"):
            try:
                ...
                _m_job = _re_num_clar.search(
                    r"(tìm|nộp|apply)\s+(\d+)\s*(job|vị trí|viec|việc)", text.lower()
                )
                ...
                from telegram import InlineKeyboardButton as _Bc
                from telegram import InlineKeyboardMarkup as _Mc

                kb = _Mc(
                    [
                        [
                            _Bc(
                                f"✅ Xác nhận tìm {_n_job} vị trí",
                                callback_data="jobsearch_confirm",
                            ),
                            _Bc("❌ Hủy", callback_data="jobsearch_cancel"),
                        ]
                    ]
                )
                await update.message.reply_text(
        ...
        is_greeting = has_email and (
            "gửi lời chào" in low
            or "gui loi chao" in low
            or ("gửi" in low and "chào" in low)
            or ("gui" in low and "chao" in low)
        )
        ...
        is_jobsearch = (
            (
                ...
                or "thực tập" in low
                or "tuc tap" in low
                or "tuyển dụng" in low
                or "tuyen dung" in low
                or "tuyển" in low
                or "tuyen" in low
                or "ai/ml intern" in low
                or "machine learning intern" in low
            )
            or (
            ...
            or (
                ("tìm" in low or "tim" in low)
                and ("còn apply" in low or "đang tuyển" in low or "còn tuyển" in low)
            )
            or ("apply được" in low and ("tìm" in low or "tim" in low or "có" in low))
            or ("ai intern" in low)
            or ("job search agent" in low)
        )
                ...
                kb2 = _M2(
                    [
                        [
                            ...
                            _B2("❌ Hủy", callback_data="jobsearch_cancel"),
                        ],
                    ]
                )
                await update.message.reply_text(
        ...
        is_youtube_trending = (
            ("youtube" in low and "trending" in low)
            or ("video trending" in low)
            or ("youtube trending agent" in low)
        )
        if is_youtube_trending:
            try:
                ...
                from telegram import InlineKeyboardButton as _By
                from telegram import InlineKeyboardMarkup as _My

                kby = _My(
                    [
                        [
                            _By("✅ Bắt đầu lấy Trending", callback_data="youtube_confirm"),
                            _By("❌ Hủy", callback_data="youtube_cancel"),
                        ]
                    ]
                )
                await update.message.reply_text(
        ...
        if is_greeting:
            try:
                ...
                if res.get("mode") == "DRY_RUN":
                    await update.message.reply_text(
                        f"⚠️ Gmail đang DRY_RUN, chưa gửi thật tới {target}. Đã thêm vào allowlist rồi thử lại."
                    )
                else:
                    await update.message.reply_text(
                        f"✅ Đã gửi lời chào từ Mai Nguyễn Bình Tân tới {target} (id {res.get('id')})"
                    )
                return
            except Exception as e:
        ...
        is_code_req = (
            ("hello world" in low2)
            or ("viết code" in low2)
            or ("code đơn giản" in low2)
            or ("viết cho tôi đoạn code" in low2)
            or ("đoạn code" in low2)
        )
        if is_code_req:
            _snippets = {
        ...
        _greet_rx = _re_greet.compile(
            r"\b(" + "|".join(_simple_greet) + r")\b", _re_greet.IGNORECASE
        )
        ...
        try:
            ...
            if (
                ...
                try:
                    ...
                    pass
                await self._kb_command(update, type("__Ctx", (), {"args": text.split()})())
                return
        ...
        try:
                ...
                try:
                    ...
                    if _summary:
                        ...
                        try:
                            ...
                            _src_urls = [
                                (_r.get("url") or "").strip() for _r in results[:5] if _r.get("url")
                            ]
            ...
            try:
                ...
                req = TaskRequest(
                    ...
                    context=TaskContext(
                        organization_id=__import__("uuid").UUID(
                            "00000000-0000-0000-0000-000000000001"
                        ),
                        channel="telegram",
                    ),
                ...
                try:
                    ...
                    if hasattr(resp, "result") and resp.result:
                        reply = str(
                            resp.result.get("summary") or resp.result.get("answer") or resp.result
                        )
            ...
            try:
                ...
                if getattr(self, "_chat_memory", None) is not None and update.effective_user:
                    _aio2.create_task(
                        self._chat_memory.log_assistant(chat_id, update.effective_user, reply)
                    )
            except Exception:
                pass
        except Exception as e:
            ...
            try:
                await self._friendly_error(update, e)
            except Exception:
                pass
