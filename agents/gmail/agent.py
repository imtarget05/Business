...
class GmailAgent:
    ...
    async def handle(self, request: TaskRequest) -> AgentResponse:
        ...
        if request.action in ("send", "draft"):
            ...
            if not to_email:
                return AgentResponse(
                    ...
                    agent=self.descriptor.qualified_name,
                    status=AgentResponseStatus.REJECTED,
                    error=ErrorDetail(code="VALIDATION_ERROR", message="payload.to required"),
                )
        ...
        if not _HAS_GOOGLE:
            return AgentResponse(
                ...
                agent=self.descriptor.qualified_name,
                status=AgentResponseStatus.REJECTED,
                error=ErrorDetail(code="CONFIG_ERROR", message="Gmail not configured"),
            )
        try:
            creds = get_google_credentials()  # type: ignore
        except Exception as e:
    ...
    async def _send_gmail(
        self, request: TaskRequest, creds: Any, is_draft: bool = False
    ) -> AgentResponse:
        ...
        try:
            ...
            return AgentResponse(
                ...
                agent=self.descriptor.qualified_name,
                status=AgentResponseStatus.SUCCESS,
                result={"sent": not is_draft, "draft": is_draft, "response": res},
            )
        except Exception as e:
            return AgentResponse(
                task_id=request.task_id,
                agent=self.descriptor.qualified_name,
                status=AgentResponseStatus.FAILED,
                error=ErrorDetail(code="GMAIL_ERROR", message=str(e)),
            )
    ...
    async def _search(self, request: TaskRequest, creds: Any) -> AgentResponse:
        ...
        try:
            ...
            return AgentResponse(
                ...
                agent=self.descriptor.qualified_name,
                status=AgentResponseStatus.SUCCESS,
                result={"query": query, "messages": msgs, "count": len(msgs)},
            )
        except Exception as e:
            return AgentResponse(
                task_id=request.task_id,
