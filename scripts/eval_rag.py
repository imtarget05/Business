...
# Demo corpus (replace with real docs via KnowledgeBase.ingest)
DOCS = {
    "refunds": "Our refunds policy: customers may request refunds within 14 days of purchase. Refunds are processed to the original payment method.",
    "shipping": "Shipping takes 3-5 business days domestically and 7-14 days internationally. Tracking numbers are sent by email.",
    "warranty": "All products include a 12-month warranty covering manufacturing defects. Warranty claims require the original receipt.",
    "pricing": "The Business plan costs 29 USD per user per month, billed annually. Enterprise pricing is negotiated with sales.",
    "support": "Support is available 24/7 via email and live chat. First response time is under 2 hours for paid plans.",
}
...
def evaluate(top_k: int, chunk_size: int, overlap: int) -> dict:
    ...
    return {"hit_rate": hits / len(QA_SET), "mrr": rr / len(QA_SET), "latency_ms": round(latency, 2)}
        ...
        for chunk_size in (100, 200, 500):
            for overlap in (20, 50):
                ...
                print(
                    ...
                    f"{m['hit_rate']:>7.2f} {m['mrr']:>6.2f} {m['latency_ms']:>7.2f}"
                )
    print(f"\nBEST: top_k={best_cfg[0]} chunk_size={best_cfg[1]} overlap={best_cfg[2]} (MRR={best:.2f})")
